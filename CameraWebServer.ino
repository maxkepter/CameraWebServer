...
struct MOTOR_PINS {
  int pinEn;   //motor speed (PWM)
  int pinIN1;  //motor dir
  int pinIN2;  //motor dir
};
...
//websocker function to hanlde camera
...
bool connectToWifiSTA() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    Serial.println("\nConnected to STA Wi-Fi");
    Serial.println(WiFi.localIP());
    isAPMode = false;
    return true;
  }

  Serial.println("\nFailed to connect STA");
  return false;
}
...
//reconnect Wifi
void reconectToWifi() {
  //every 10s check connect if in STA
  if (!isAPMode && millis() - lastWifiCheck > wifiReconnectInterval) {
    lastWifiCheck = millis();

    //reconnect STA
    if (WiFi.status() != WL_CONNECTED) {
      Serial.println("WiFi disconnected, attempting reconnect...");
      bool connected = connectToWifiSTA();

      //start Ap if can not reconnect
      if (!connected) {
